...
def train(ddpm: DDPM, net, device='cuda', ckpt_path='dldemos/ddpm/model.pth'):
    ...
    tic = time.time()
#   代码的主要逻辑都在循环里。首先是完成训练数据x_0,t,噪声的采样。采样x_0的工作可以交给PyTorch的DataLoader完成，每轮遍历得到的x就是训练数据。 t的采样可以用torch.randint函数随机从[0, n_steps - 1]取数。采样高斯噪声可以直接用torch.randn_like(x)生成一个和训练图片x形状一样的符合标准正态分布的图像。

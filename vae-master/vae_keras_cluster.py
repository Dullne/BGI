...
classfier = Model(z, y,name='classfier') # 隐变量分类器
...
x_recon = decoder(z)
y = classfier(z)
encoder = Model(x, z,name='encoder') # 通常认为z_mean就是所需的隐变量编码
...
class Gaussian(Layer):
    ...
    def call(self, inputs):
        ...
        z = K.expand_dims(z, 1)
        return z - K.expand_dims(self.mean, 0)   #z-(1,num_classes,latent_dim)= (batch_size, num_classes, latent_dim) 
    def compute_output_shape(self, input_shape):
        return (None, self.num_classes, input_shape[-1])

gaussian = Gaussian(num_classes)
z_prior_mean = gaussian(z)  #z-\mu_y
print(z_prior_mean)

# 建立模型
with tf.name_scope("VAE"):
    vae = Model(x, [x_recon, z_prior_mean, y],name='vae')
...
# 设置 TensorBoard 回调
log_dir = "logs/vae_cluster/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
def cluster_sample(path, category=0):
    ...
    # Convert the figure to 8-bit unsigned integers

...
class celltype_GRN_model:
    ...
    def init_data(self,):
        ...
        data_values = data.X
        Dropout_Mask = (data_values != 0).astype(float) #Dropout_Mask指示哪些元素在原始 data_values 中是非零的。
        means = []
        stds = []
        ...
        num_genes, num_nodes = data.shape[1], data.shape[0]          #910    758
        ...
        for i, item in enumerate(data.columns):
            for j, item2 in enumerate(data.columns):
                ...
                if item2 in TF and item in All_gene:
                    Evaluate_Mask[i, j] = 1                  #所有可能的调控关系  343*516
                if item2 in TF:
                    TF_mask[i, j] = 1                        #有TF的列的全部为1   343*909
        pd.DataFrame(TF_mask,index=data.columns, columns=data.columns).to_csv('TF_mask.csv')
        ...
        print("样本：，dropoutmask:")
        print(train_data.tensors[0].shape,train_data.tensors[1].shape,train_data.tensors[2].shape)
        ...
        for i in range(Ground_Truth.shape[0]): #真正的调控网络   4257
            truth_df.loc[Ground_Truth.iloc[i, 1], Ground_Truth.iloc[i, 0]] = 1   #列是TF 行是factor
        A_truth = truth_df.values
        idx_rec, idx_send = np.where(A_truth) #返回A_truth中所有非零元素的索引。第一个数组是所有非零元素的行索引，第二个数组是相应的列索引。
    ...
    def train_model(self):
        ...
        adj_A_init  = self.initalize_A(data)#构造的初始权重矩阵
        print(adj_A_init)
        ...
        for epoch in range(self.opt.n_epochs+1):
            ...
            for i, data_batch in enumerate(dataloader, 0):
                ...
                print("数据的输入：")
                print(inputs.shape,dropout_mask.shape)
                loss, loss_rec, loss_gauss, loss_cat, dec, y, hidden = vae(inputs,dropout_mask=dropout_mask,temperature=temperature,opt=self.opt)
                sparse_loss = self.opt.alpha * torch.mean(torch.abs(vae.adj_A)) #W 的 L1 范数的损失系数，与我们论文中使用的 \\alpha 相同。

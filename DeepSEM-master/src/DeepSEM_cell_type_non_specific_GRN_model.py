...
class non_celltype_GRN_model:
    ...
    def init_data(self):
        ...
        # pd.DataFrame(Dropout_Mask).to_csv('Dropout_Mask.csv')
        data_values = (data_values - data_values.mean(0)) / (data_values.std(0))#标准化
        print(len(list(data.obs_names)))
        ...
        # print(len(TF),len(All_gene),num_genes)  #343 517 910
        Evaluate_Mask = np.zeros([num_genes, num_genes])   #910*910
        ...
        # pd.DataFrame(feat_train).to_csv('feat_train.csv')
        ...
        for i in range(Ground_Truth.shape[0]):
            truth_df.loc[Ground_Truth.iloc[i, 1], Ground_Truth.iloc[i, 0]] = 1   #真正的调控网络   4257
        # pd.DataFrame(Evaluate_Mask,index=data.columns, columns=data.columns).to_csv("Evaluate_Mask.csv")
        # truth_df.to_csv("truth_df.csv")
    ...
    def train_model(self):
        ...
        print(dropout_mask.shape,inputs.shape,inputs[0].shape)
